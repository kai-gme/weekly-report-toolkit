...
def status(v, cls, note):
    ...
    if not v:
        return ""
    done, todo = checklist(note)
    total = len(done) + len(todo)
    who = todo[0][1] if todo else ""
...
def checklist(txt):
    """비고 셀의 [x]/[ ] 줄을 판다 → (완료 리스트, 미완료 리스트). 각 원소 = (항목, 담당, 주차).
    ...
    done = [split_item(m.group(2)) for m in hit if m.group(1) != " "]
...
def history(txt):
    ...
    if box:
        def item(l):
            ...
            return f'<div>{"☑" if m.group(1) != " " else "☐"} {body}</div>'
...
def main(path):
    ...
    for d in rows:
        ...
        d["done"], d["todo"] = checklist(d["steps"])
    ...
    sections = [
        ...
        ("진행 중 (In Progress)",
         [(d, f'{d["todo"][0][0]}{" — " + d["todo"][0][1] if d["todo"][0][1] else ""}'
              f'{" / " + d["log"] if d["log"] else ""}', bool(d["todo"][0][1]))
          for d in moved if d["todo"]]
         # 단계를 안 쪼갠 과제는 로그 줄이 곧 '이번 주 한 일'
         + [(d, d["log"], False) for d in moved
            if not d["todo"] and not d["done"] and str(d["status"]).strip() != "완료"]),
        ...
        ("다음 주 계획 (Plan)",
         ...
         + [(d, f'{d["todo"][1][0]}{" — " + d["todo"][1][1] if d["todo"][1][1] else ""}',
             bool(d["todo"][1][1]))
            for d in moved if len(d["todo"]) > 1 and not nextline(d["note"])]),
    ]
...
<p class="none" style="font-size:12px">원본: <code>{esc(os.path.basename(path))}</code> — 이 HTML 은 엑셀에서 자동 생성됩니다. 직접 수정하지 마세요.</p>
    ...
    for d in rows:
        want = d["todo"][0][1] if d["todo"] else ""
